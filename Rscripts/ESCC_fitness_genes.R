...
length(which(rowSums (ESCC.fit.genes[,-1]) >= 17))
#[1] 718
length(which(rowSums (ESCC.fit.genes[,-1]) >= 16))
#[1] 839
...
##==========================
ESCC.genes.any <- ESCC.fit.genes[which(rowSums (ESCC.fit.genes[,-1]) >= 1),]
head(ESCC.genes.any)
dim(ESCC.genes.any)

sortRow <- ESCC.genes.any[order (-rowSums(ESCC.genes.any[,-1])),]
r.nam  <- sortRow[,1]

temp  <- sortRow[,-1]
temp.1<- temp [, order(-colSums(temp))]

tail(temp.1)
colSums(temp.1[,c(1:2)])
sortCol <- cbind (r.nam, temp.1)
dim(sortCol)

write.csv(sortCol, "X:/project2020/MustARD/firstPass/fitness_heatmap.csv", row.names = FALSE)
heatmap (as.matrix(sortCol[,-1]))
heatmap (as.matrix(sortCol[,-1]),  Rowv = NULL, Colv = NULL, ) 
library(gplots)
heatmap.2(as.matrix(sortCol[,-1]),dendrogram='none', Rowv=FALSE, Colv=FALSE,trace='none')
##===========================
